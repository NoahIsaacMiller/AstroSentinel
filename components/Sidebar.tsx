...
import { Disc, AlertTriangle, Radio, Satellite, Hexagon, Search, LocateFixed, Signal, MapPin } from 'lucide-react';
           ...
           <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2">
                 <div className="bg-slate-950 p-2 rounded border border-slate-800">
                    <div className="text-slate-500 mb-0.5">{d.latitude}</div>
                    <div className="text-cyan-100 font-bold text-xs">{telemetry.lat.toFixed(4)}°</div>
                 </div>
                 <div className="bg-slate-900 p-2 rounded border border-slate-800">
                    <div className="text-slate-500 mb-0.5">{d.longitude}</div>
              ...
              <div className={`p-2 rounded border flex items-center justify-between transition-colors
                 ...
                 <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                       <span className="text-slate-500 font-bold">{d.groundLink}</span>
                       <span className={`${visibleStation ? 'text-green-400' : 'text-slate-600'} font-bold`}>
                          {visibleStation ? visibleStation : d.noLink}
                       </span>
                    </div>
                 </div>
                 {visibleStation && <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_5px_lime]"></div>}
              </div>
           </div>
         ) : (
